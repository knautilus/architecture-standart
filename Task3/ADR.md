...
- Для кэширования данных можно использовать memcached, однако он не поддерживает персистентность (сохранение данных при перезагрузке).

**Недостатки, ограничения, риски**

- Зависимость от монолита интернет-банка, который сложно поддерживать и невозможно модифицировать на уровне ядра. Имеет смысл перевести его на микросервисную архитектуру.
- АБС может масштабироваться только вертикально, что ограничивает способность системы обрабатывать большие объёмы заявок.
- Использование различных СУБД и необходимость реплицировать данные между ними усложняют разработку.
